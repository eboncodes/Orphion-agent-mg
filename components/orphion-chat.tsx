...
export default function OrphionChat({
  ...
  const processUserMessage = async (userPrompt: string, currentSession: ChatSession, currentMessages: Message[]) => {
    ...
    try {
      ...
      let response: string, reasoning: string | undefined, webSearchMetadata: any
      ...
      // Handle "Another response is already being generated" error
      if (errorMessage === "Another response is already being generated") {
        console.log("Skipping duplicate message generation")
        return currentSession
      }
      ...
      const sessionWithError = addMessage(currentSession, errorResponse, "ai", {
        reasoning: errorMessage,
        generationTime: Math.round((Date.now() - startTime) / 1000),
      })

      // Update state with the last message from the session
      const lastMessage = sessionWithError.messages[sessionWithError.messages.length - 1]
      setSession(sessionWithError)
      setMessages([...currentMessages, lastMessage])

      // Notify parent if needed
      if (isPendingCreation && onChatCreated) {
        onChatCreated(sessionWithError)
        setIsProcessingInitialMessage(false)
      } else if (onChatUpdated) {
        onChatUpdated(sessionWithError)
      }
  ...
  useEffect(() => {
    ...
    if ((initialMessage || initialAttachedImage) && isPendingCreation && !isProcessingInitialMessage && !session && !isProcessingRef.current) {
      // Check if both API keys exist
      const { groq: missingGroq, tavily: missingTavily } = getMissingApiKeys()
      ...
      // Set flags to prevent multiple processing
